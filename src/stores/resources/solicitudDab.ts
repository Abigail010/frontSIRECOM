  ...
  actions: {
    ...
    async dabs () {
      try {
        ...
        const { data } = await siibApi.get('dab/solicitudes')
        return data
      } catch (error: any) {
    ...
    //OBTENER LA LISTA DE BIENES DISPONIBLES 
    async bienes (form: any) {
  /// console.log("id del bien : "+ form)
      try {
        const { data } = await siibApi.post('dab/bien/'  + userLogged, form)
        return data
      } catch (error: any) {
        const message = (error.response.data ? error.response.data.message : 'error: sin conexion')
        return { ok: false, message: message }
