...
const getDepositsList = async() => {
    desserts.value = await depositStore.deposits() 
}

  const buttonDepositForm = (id_deposito: any) => {
    router.push({ name: 'depositForm', params: { id_deposito: id_deposito }})
  }

  // nuevo data table
const headers = ref([
  { title: 'Acciones', key: 'actions', sortable: false },
  { title: 'Depositos', key: 'nombre_deposito' },
  { title: 'Departamento', key: 'departamento' },
  { title: 'Direccion', key: 'direccion_deposito' },
  { title: 'Enlace', key: 'url_deposito' },
])
  ...
  function deleteItem(item: any) {
    ...
    if (result.isConfirmed) {
      const { ok, message } = await depositStore.deleteDeposit({"id":item})
